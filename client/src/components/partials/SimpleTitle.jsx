...
const SimpleTitle = ({ title, stickyStatus = true, backgroundColor, fontSize, uppercase, darkMode = false }) => {
  ...
  return (
    <h1
      ...
      <span
        translate="no"
        className={`absolute left-0 top-0 right-0 ${darkMode ? 'text-shadow-glow-purple text-purple-soft' : 'text-purple-soft'}  z-[2] translate-x-0.5 translate-y-0.5`}>{title}</span>
      <span
        translate="no"
        className={`absolute left-0 top-0 right-0 ${darkMode ? 'text-shadow-glow-green text-green-soft' : 'text-green-soft'}  z-[1] translate-x-1 translate-y-1`}>{title}</span>
    </h1>
  );
};
