...
const HalfPenaltyDay = ({ rule, user, viewedUser, isOwnProfile }) => {
  ...
  return (
    <div className="block relative z-20 flex flex-col justify-center items-center mb-4">
      <div className="w-full -rotate-[5deg] relative">
        <AnimatedTitle title="Noyeux Joel Don Markus" darkMode={true} stickyStatus={false} />
      </div>
      <Link
      ...
      >
        <span
          translate="no"
          className="relative z-[2] w-full block border border-black text-black uppercase font-regular text-sm font-roboto px-3 py-1 rounded-full text-center shadow-md bg-green-deep transition -translate-y-1 group-hover:-translate-y-0"
        >
          Voir la vidéo
        </span>
      </Link>
