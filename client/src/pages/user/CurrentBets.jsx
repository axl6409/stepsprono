          ...
          <div className="flex flex-col w-full">
            ...
            <div className={`flex flex-col mt-2 border-2 border-black rounded-xs shadow-flat-black-adjust`}>
              {matchs.map((bet, index) => (
                <div key={index} className="flex flex-row py-1 mx-0.5 border-b last-of-type:border-0 border-black even:bg-sky-50">
                  <div className="w-[5%] p-1">
                    <p className="font-title text-base font-bold inline-block align-sub">{index}</p>
                  </div>
                  <div className="w-[25%] p-1">
                    <div className="flex flex-row justify-between">
                      <img className="inline-block h-auto w-8" src={bet.Match.HomeTeam.logoUrl} alt={bet.Match.HomeTeam.name} />
                      <span className="font-sans font-bold text-center block my-auto"> - </span>
                      <img className="inline-block h-auto w-8" src={bet.Match.AwayTeam.logoUrl} alt={bet.Match.AwayTeam.name} />
                    </div>
                  </div>
